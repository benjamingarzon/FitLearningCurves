...
mytrials = mytrials %>% arrange(username, sess_day, true_sequence, trial)%>%filter(trial>10)
...
if (T){
fit.1 = stan(file = "learning_model_constant3comp_HR.stan", chains = 4, iter = ITER, warmup = WARMUP, data = stan_data, control = stan_control, init = stan_init)
} else {
fit.1 = vb(stan_model(file = "learning_model_constant3comp_HR.stan"), iter = ITER, data = stan_data, init = stan_init)
}
...
traceplot(fit.1, par = c('mu_hyp_csub', 'sigma_hyp_csub','sigma', 'mu_hyp_beta', 'sigma_hyp_beta', 'mu_hyp_seq', 'sigma_hyp_seq'), inc_warmup = T)
traceplot(fit.1, par = c( 'mu_hyp_csub', 'sigma_hyp_csub', 'sigma'))
plot(fit.1, par = c('mu_hyp_csub', 'sigma_hyp_csub'))
plot(fit.1, par = c('mu_hyp_seq',  'sigma_hyp_seq'))
pairs(fit.1, pars = c( 'mu_hyp_csub[1]', 'mu_hyp_csub[3]', 
                       'sigma_hyp_csub[1]', 'sigma_hyp_csub[3]'))
pairs(fit.1, pars = c('mu_hyp_csub', 'sigma_hyp_csub'))
#check parameters
