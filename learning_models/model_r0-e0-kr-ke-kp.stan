// 3 compartment model fixing p0 to 0, assumes that measures have reached stability within a session
...
parameters{
  row_vector[5] mu_hyp_csub; //r0, e0, kr, ke, kp
  
  row_vector<lower = 0>[5] sigma_hyp_csub;
...
transformed parameters{
  ...
  for (j in 1:Nclus){
    ...
    kr = Phi_approx(5*csub[subject[j], 3]);
    ke = Phi_approx(5*csub[subject[j], 4]);
    kp = Phi_approx(5*csub[subject[j], 5]);
    kc = ke + kp;
    aux0 = 0;
    aux1 = 0;
    ...
    c[j] = 1000*inv(r0*kr*kp*aux0/kc + r0*kr*ke*aux1/kc + ke*e0*exp(-kc*day[j])/kc + kp*e0/kc); // transform capacity into movement time
  }
...
generated quantities{
    ...
    r0_c = exp(csub[, 1]);
    e0_c = exp(csub[, 2]);
    kr_c = Phi_approx(5*csub[, 3]);
    ke_c = Phi_approx(5*csub[, 4]);
    kp_c = Phi_approx(5*csub[, 5]);

    retention_c = kp_c ./ (ke_c + kp_c);

    r0_c_mean = exp(mu_hyp_csub[1]);
    e0_c_mean = exp(mu_hyp_csub[2]);
    kr_c_mean = Phi_approx(5*mu_hyp_csub[3]);
    ke_c_mean = Phi_approx(5*mu_hyp_csub[4]);
    kp_c_mean = Phi_approx(5*mu_hyp_csub[5]);
