...
parameters{
  row_vector[4] mu_hyp_csub; //r0, e0, kr, kp
...
transformed parameters{
  ...
  for (j in 1:Nclus){
    ...
    kp = Phi_approx(5*csub[subject[j], 4]);
    aux0 = 0;
    aux1 = 0;
    ...
    for (i in 1:(cum_day[j] - 1)) { 
      aux1 = aux1 + (1 - kr)^(i - 1)*exp(-kp*(day[j] - time_mat[subject[j], i]));
      }
    c[j] = 1000*inv(r0*kr*kp*aux0/kp + kp*e0/kp); // transform capacity into movement time
  }
