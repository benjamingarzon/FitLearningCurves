...
//  vector[Ncovars] beta;
  ...
  matrix[Nsub, Nseq] seq_offset_r; // differences in difficulty
...
//  matrix[Nsub, 3] dsub;
  ...
  for (k in 1:Nsub){
    ...
    seq_offset[k, ] = mu_hyp_seq + sigma_hyp_seq .* seq_offset_r[k, ];
  }
  ...
  for (j in 1:Nclus){
    ...
    c[j] = 1000*inv(r0*kr*aux + c0*exp(-kr*day[j])); // transform capacity into movement time
  }
